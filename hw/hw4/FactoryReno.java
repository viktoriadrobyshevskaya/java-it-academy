...
public class FactoryReno extends Factory {
    private List<Car> warehouse;

    public FactoryReno() {
        warehouse = new ArrayList<>();
        warehouse.add(new CarReno(Label.RENO, Color.GREEN, 2019, WheelSize.LARGE, Capacity.LARGE));
        warehouse.add(new CarReno(Label.RENO, Color.RED, 2021, WheelSize.SMALL, Capacity.SMALL));
        warehouse.add(new CarReno(Label.RENO, Color.GREY, 2020, WheelSize.MEDIUM, Capacity.MEDIUM));
    }
    ...
    public Car createCar(Color color, int year, WheelSize size, Capacity capacity) {
        ...
        System.out.println("Салон получает машину:");
        return new CarReno(Label.RENO, color, year, size, capacity);
    }
